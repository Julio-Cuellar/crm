...
from app.interfaces.api.schemas.appointment import (
    AppointmentCreate,
    AppointmentUpdate,
    AppointmentResponse,
...
from app.interfaces.api.dependencies.appointments import (
...
async def _enrich_appointments(db: AsyncSession, appointments: list) -> list[AppointmentResponse]:
    ...
    # 2. Cargar clientes en bloque
    customers_map = {}
    if customer_ids:
        res_cust = await db.execute(select(DbCustomer).where(DbCustomer.id.in_(list(customer_ids))))
        customers_map = {c.id: c for c in res_cust.scalars().all()}

    # 3. Cargar servicios en bloque
    services_map = {}
    if service_ids:
        res_serv = await db.execute(select(DbService).where(DbService.id.in_(list(service_ids))))
