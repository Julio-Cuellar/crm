...
from app.modules.conversations.infrastructure.gateways.tenants_fallback_gateway import TenantsFallbackGateway
from app.modules.conversations.infrastructure.messaging.publishers import publish_chat_inbound_simulated
from app.modules.conversations.interfaces.api.schemas.chat import ChatResponse, MessageResponse, SendMessageRequest, ChatModeUpdateRequest
from app.platform.whatsapp.whatsapp_cloud_api import (
...
async def _get_customer_or_404(db: AsyncSession, customer_id: uuid.UUID, tenant_id: uuid.UUID) -> CustomerProjection:
    repo = CustomerProjectionRepository(db)
...
async def list_chats(
    ...
    # 2. Cargar Customers en bloque (desde la proyección local, no la tabla de `customers`)
    projection_repo = CustomerProjectionRepository(db)
    customers_map = {c.id: c for c in await projection_repo.get_by_ids(customer_ids)}

    # 3. Cargar BotSessions en bloque
    from app.modules.conversations.infrastructure.db.models.bot_session import BotSession as DbBotSession
...
async def get_chat_messages(
    ...
    tenant_id = current_user.tenant_id
    await _get_customer_or_404(db, customer_id, tenant_id)
...
async def send_chat_message(
    ...
    tenant_id = current_user.tenant_id
    customer = await _get_customer_or_404(db, customer_id, tenant_id)

    gateway = TenantsFallbackGateway(token)
    phone_number_id, access_token = await gateway.get_whatsapp_config()
...
async def receive_incoming_message_sim(
    ...
    if settings.BOT_ENABLED_DEFAULT and message_in.content:
        ...
        is_human_mode = bot_session and bot_session.automation_mode == "HUMAN"

        if not is_human_mode:
            # `assistant` (que ejecuta el dispatch real al bot) sigue en app/legacy/,
            # así que `conversations` no puede importarlo directamente — publica el
            # evento y `assistant` reproduce el mismo dispatch en su propio consumer.
            await publish_chat_inbound_simulated(
                event_bus,
                tenant_id=tenant_id,
